...
- Boolean-операторов (AND / OR / NOT / группировка) с **критичным**
  ...
  PowerShell)
- Ограничения по пути (`-path`, `-parent`, `path:`, `-p`) — включая ловушку
  с одной буквой диска: `-path D:` означает **текущий каталог** на этом
  диске, а не весь диск (весь диск — `-path "D:\"`), и молча недобирает
  результаты
